from pygltflib import GLTF2, Scene, Mesh, Primitive, Node, Buffer, BufferView, Accessor, Attributes, UNSIGNED_BYTE, SCALAR, FLOAT, VEC3, ELEMENT_ARRAY_BUFFER, ARRAY_BUFFER
import numpy as np
...
class GLTFBuilder:
    ...
    def add_triangular_mesh(self, triangular_mesh: TriangularMesh):
        ...
        A new node with this mesh will be added to the scene.
        ...
        self._add_buffer()        
        self._add_buffer_view(triangles_binary_blob, buffer_index, target=ELEMENT_ARRAY_BUFFER)
    ...
    def _add_mesh(self):
        ...
        primitive = Primitive(attributes=Attributes(POSITION=position_accessor_index), indices=indices_accessor_index) 
    ...
    def _add_buffer_view(self, blob: bytes, buffer_index: int, target: int):
        ...
        if len(self._gltf.bufferViews) == 0:
            ...
            byte_offset = previous_buffer_view.byteOffset + previous_buffer_view.byteLength
